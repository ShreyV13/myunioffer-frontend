...
function RadarChart({ scores, size = 220 }) {
  ...
  const angleStep = (2 * Math.PI) / 5, off = -Math.PI / 2;
  const polar = (a, v) => { const r = (v / maxVal) * (size * 0.38); return { x: cx + r * Math.cos(a + off), y: cy + r * Math.sin(a + off) }; };
  const pts = CATEGORY_KEYS.map((k, i) => polar(i * angleStep, scores[k] || 0));
  const path = pts.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x},${p.y}`).join(' ') + 'Z';
  return (
    <svg width={size} height={size} className="mx-auto">
      {[5, 10, 15, 20].map(lv => { const lps = CATEGORY_KEYS.map((_, i) => polar(i * angleStep, lv)); return <path key={lv} d={lps.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x},${p.y}`).join(' ') + 'Z'} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth={1} />; })}
...
export default function RateMyPS() {
  ...
  const [pageState, setPageState] = useState('input'); // input | scoring | results
  ...
  function handleSubmit() {
    ...
    setPageState('scoring'); apiReturned.current = false; setResult(null); setLanded(false); setProgress(0);
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      // Progress bar
      const raw = totalElapsed / 35000;
      const max = hasResult ? 1 : 0.92;
      setProgress(Math.min(1 - Math.pow(1 - raw, 2.5), max) * 100);

      // Range narrows continuously
      if (!hasResult) {
        currentRange = Math.max(currentRange - 0.12, 30);
      } else {
        currentRange = Math.max(currentRange - 1.8, 0);
      }
  ...
  function handleReset() {
    setPageState('input'); setResult(null); setDisplayNumber(0); setError(''); setProgress(0); setLanded(false); setShowPsText(false);
  }
    ...
    <div className="min-h-screen flex flex-col" style={{ background: '#1a1a1a' }}>
      ...
      {pageState === 'input' && (
        <div className="flex-1 flex flex-col items-center px-6 py-10 lg:py-16 relative z-10">
          <div className="w-full max-w-2xl">
            ...
            {/* Category pills */}
            <motion.div className="flex flex-wrap justify-center gap-2 mb-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15, duration: 0.4 }}>
            ...
            {/* Subject input */}
            <motion.div className="mb-3 relative" ref={subjectRef} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.4 }}>
            ...
            {/* Textarea */}
            <motion.div className="relative mb-4" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.4 }}>
            ...
            <p className="text-xs text-white/20 text-center mt-3">
              {currentUser ? 'Free to use. No limit deducted until results load.' : 'Create a free account first. Takes 10 seconds.'}
            </p>

            {/* Trust bar */}
            <motion.div className="mt-10 flex flex-wrap items-center justify-center gap-6 text-xs text-white/25"
      ...
      {pageState === 'results' && result && (
        <div className="flex-1 flex flex-col lg:flex-row relative z-10">
          ...
          {/* Left: PS text (desktop) / collapsible (mobile) */}
