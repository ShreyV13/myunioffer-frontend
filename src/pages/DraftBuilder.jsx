...
import { GraduationCap, ArrowRight, ArrowLeft, Sparkles, AlertTriangle, Check, ChevronDown, ChevronUp, Loader2, Settings, Crown, BookOpen, Briefcase, Star, Lightbulb, Zap, PenTool, X, RotateCcw } from 'lucide-react';
        ...
        <React.Fragment key={i}>
          <div className="flex flex-col items-center gap-1.5">
            <motion.div
              ...
              style={{
                background: i <= current ? 'rgba(249,106,80,0.15)' : 'rgba(255,255,255,0.04)',
                color: i <= current ? '#f96a50' : 'rgba(255,255,255,0.2)',
                border: `1.5px solid ${i <= current ? 'rgba(249,106,80,0.4)' : 'rgba(255,255,255,0.08)'}`,
              }}
...
function FloatingCard({ card, onClick, isAssigned, isPriority, onStar, onRemove, delay = 0 }) {
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            border: `1px solid ${isPriority ? 'rgba(251,191,36,0.4)' : 'rgba(255,255,255,0.08)'}`,
            boxShadow: isPriority ? '0 0 20px rgba(251,191,36,0.1)' : 'none',
        ...
        >
          ...
          <div className="flex items-center gap-3 pl-2">
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              {card.detail && (
                <button onClick={e => { e.stopPropagation(); setFlipped(true); }} className="w-6 h-6 rounded-lg flex items-center justify-center hover:bg-white/5">
                  <RotateCcw className="w-3 h-3 text-white/30" />
                </button>
              )}
              {isAssigned && onStar && (
                <button onClick={e => { e.stopPropagation(); onStar(); }} className="w-6 h-6 rounded-lg flex items-center justify-center hover:bg-white/5">
                  <Star className="w-3 h-3" style={{ color: isPriority ? '#fbbf24' : 'rgba(255,255,255,0.2)', fill: isPriority ? '#fbbf24' : 'none' }} />
                </button>
              )}
              {isAssigned && onRemove && (
                <button onClick={e => { e.stopPropagation(); onRemove(); }} className="w-6 h-6 rounded-lg flex items-center justify-center hover:bg-white/5">
        ...
        {/* Back (reflection) */}
        <div
          className="absolute inset-0 rounded-2xl px-4 py-3 overflow-hidden"
          style={{
...
function SectionReceiver({ section, cards, onDrop, onStar, onRemove }) {
  ...
  return (
    <motion.div
      ...
      style={{
        background: meta.bg,
        border: `1.5px solid ${cards.length > 0 ? meta.border : 'rgba(255,255,255,0.06)'}`,
      }}
    ...
    >
      ...
      {cards.length > 0 && (
        <div className="absolute inset-0 pointer-events-none" style={{
          background: `radial-gradient(ellipse at 50% 0%, ${meta.glow} 0%, transparent 60%)`,
        }} />
      )}
      ...
      <div className="h-1 rounded-full mb-3 relative z-10" style={{ background: 'rgba(255,255,255,0.04)' }}>
        <motion.div className="h-full rounded-full" style={{ background: meta.color, width: `${fillPct}%` }}
          initial={{ width: 0 }} animate={{ width: `${fillPct}%` }} transition={{ duration: 0.5, ease: 'easeOut' }} />
      </div>
      ...
      <div className="space-y-2 relative z-10 min-h-[48px]">
        <AnimatePresence mode="popLayout">
          {cards.map((card, i) => (
            <FloatingCard key={card.id || card.title} card={card} isAssigned onStar={() => onStar(card)} onRemove={() => onRemove(card)} isPriority={card.priority} delay={0} />
          ))}
        </AnimatePresence>
        {cards.length === 0 && (
          <div className="text-center py-4">
            <p className="text-[10px] italic" style={{ color: meta.color, opacity: 0.3 }}>Click cards above to add here</p>
          </div>
        )}
      </div>
    </motion.div>
...
function ScaffoldSection({ sectionKey, text, annotations, expanded, onToggle }) {
  ...
  const cleanCharCount = (text || '').replace(/\[EXPAND[^\]]*\]/g, '').length;
  const fillPct = Math.min(cleanCharCount / meta.target * 100, 100);
  ...
  return (
    ...
    >
      ...
      <button className="w-full px-5 py-4 flex items-center justify-between relative z-10" onClick={onToggle}
        style={{ background: expanded ? meta.bg : 'rgba(255,255,255,0.02)' }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-[10px] tabular-nums" style={{ color: meta.color }}>{cleanCharCount}/{meta.target}</span>
            </div>
          </div>
        </div>
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {sectionAnns.map((ann, i) => (
                    <span key={i} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium"
                      style={{
                        background: ann.type === 'strength' ? 'rgba(52,211,153,0.1)' : ann.type === 'gap' ? 'rgba(251,191,36,0.1)' : 'rgba(248,113,113,0.1)',
                        color: ann.type === 'strength' ? '#34d399' : ann.type === 'gap' ? '#fbbf24' : '#f87171',
                        border: `1px solid ${ann.type === 'strength' ? 'rgba(52,211,153,0.2)' : ann.type === 'gap' ? 'rgba(251,191,36,0.2)' : 'rgba(248,113,113,0.2)'}`,
                      }}>
...
export default function DraftBuilder() {
  ...
  const [step, setStep] = useState(0); // 0=brief, 1=builder, 2=draft
  ...
  // Builder state
  const [pool, setPool] = useState([]); // unassigned cards
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/get-ps-brief`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: currentUser.uid }) })
      ...
      .then(data => {
        setBrief(data);
        const cards = (data.cards || []).map((c, i) => ({ ...c, id: `card_${i}`, priority: false }));
        setPool(cards);
        setLoading(false);
      })
  ...
  function assignCard(card, sectionKey) {
    setPool(prev => prev.filter(c => c.id !== card.id));
    setAssigned(prev => ({ ...prev, [sectionKey]: [...prev[sectionKey], { ...card, priority: false }] }));
    setAssignModal(null);
  }

  function removeCard(card, sectionKey) {
    setAssigned(prev => ({ ...prev, [sectionKey]: prev[sectionKey].filter(c => c.id !== card.id) }));
  ...
  function toggleStar(card, sectionKey) {
    setAssigned(prev => ({
      ...prev,
      [sectionKey]: prev[sectionKey].map(c => c.id === card.id ? { ...c, priority: !c.priority } : c)
    }));
  }
  ...
  function handleBuild() {
    ...
    for (const [key, cards] of Object.entries(assigned)) {
      if (cards.length > 0) {
        sectionAssignments[key] = cards.map(c => ({ title: c.title, detail: c.detail || '', priority: c.priority }));
      }
    }
    fetch(`${API_BASE}/generate-ps`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user_id: currentUser.uid, tier, section_assignments: Object.keys(sectionAssignments).length > 0 ? sectionAssignments : null }),
    })
  ...
  return (
    <div className="min-h-screen flex flex-col relative" style={{ background: '#111113' }}>
      {/* Grid background */}
      ...
      {!loading && error && (
        <div className="flex-1 flex items-center justify-center px-6 py-16">
          <div className="text-center max-w-lg">
            ...
            <p className="text-sm text-white/50 leading-relaxed mb-6">The Draft Builder turns your coaching conversations into a structured starting point. Chat with the coach about your subject, experiences, and reflections first.</p>
            <div className="rounded-xl p-5 mb-6 text-left" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              {['Tell the coach what subject you are applying for', 'Share your experiences: work, volunteering, projects, reading', 'Reflect on what those experiences taught you', 'Come back here when you have enough material'].map((s, i) => (
                <div key={i} className="flex items-start gap-3 mb-2.5">
                  <span className="w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-bold flex-shrink-0 mt-0.5" style={{ background: 'rgba(249,106,80,0.15)', color: '#f96a50' }}>{i + 1}</span>
            ...
            {step === 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <div className="text-center mb-8">
                  ...
                  <p className="text-white/35 text-sm">Everything the coach has gathered from your conversations. Review it, then arrange it into your PS structure.</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-8">
                  <AnimatePresence>
                    {pool.map((card, i) => (
                      <FloatingCard key={card.id} card={card} delay={i * 0.03} onClick={() => {}} />
                    ))}
                  </AnimatePresence>
                </div>

                <div className="text-center">
                  <button onClick={() => setStep(1)} className="gradient-primary text-white font-semibold py-3.5 px-8 rounded-xl text-sm shadow-lg shadow-coral-500/20 hover:shadow-xl hover:-translate-y-0.5 transition-all inline-flex items-center gap-2">
                    Arrange into sections <ArrowRight className="w-4 h-4" />
                  </button>
                </div>
              </motion.div>
            )}
            ...
            {step === 1 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <div className="text-center mb-6">
                  ...
                  <p className="text-white/35 text-sm">Click a card to place it in a section. Star your strongest material. The AI will follow your arrangement.</p>
                </div>
                ...
                {pool.length > 0 && (
                  <div className="mb-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <AnimatePresence mode="popLayout">
                        {pool.map(card => (
                          <FloatingCard key={card.id} card={card} onClick={() => setAssignModal(card)} />
                        ))}
                      </AnimatePresence>
                    </div>
                  </div>
                  ...
                  {assignModal && (
                    <motion.div className="fixed inset-0 z-[100] flex items-center justify-center px-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      ...
                      <motion.div className="relative rounded-2xl p-6 max-w-sm w-full" style={{ background: '#1a1a1c', border: '1px solid rgba(255,255,255,0.1)' }}
                        ...
                        <p className="text-sm font-semibold text-white mb-1">Where does this go?</p>
                        <p className="text-xs text-white/35 mb-4 truncate">{assignModal.title}</p>
                        <div className="space-y-2">
                          {Object.entries(SECTIONS).map(([key, meta]) => (
                            <button key={key} onClick={() => assignCard(assignModal, key)}
                              ...
                              <span className="text-xs font-semibold" style={{ color: meta.color }}>{meta.title}</span>
                              <span className="text-[10px] ml-auto" style={{ color: meta.color, opacity: 0.4 }}>{assigned[key].length} items</span>
                            </button>
                          ))}
                        </div>
                        <button onClick={() => setAssignModal(null)} className="w-full text-center text-xs text-white/25 mt-3 py-2 hover:text-white/40 transition-colors">Cancel</button>
                      </motion.div>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <div className="space-y-3 mb-8">
                  {Object.keys(SECTIONS).map(key => (
                    <SectionReceiver key={key} section={key} cards={assigned[key]}
                      onDrop={(card) => assignCard(card, key)}
                      onStar={(card) => toggleStar(card, key)}
                      onRemove={(card) => removeCard(card, key)} />
                  ))}
                </div>
            ...
            {step === 2 && scaffold && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <div className="mb-6 rounded-xl px-4 py-3 flex items-start gap-3" style={{ background: 'rgba(96,165,250,0.05)', border: '1px solid rgba(96,165,250,0.12)' }}>
                  ...
                  <p className="text-xs text-blue-300/70">This is a scaffold built from your material, not a finished statement. Expand each section, replace generic phrasing with your own words, and use Rate My PS to check your score.</p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => { setScaffold(null); handleBuild(); }} disabled={scaffoldLoading}
